...
| Date | Passe |
|---|---|
| **2026-07-29** | **Atelier Scènes v1** (branche `feat/atelier-scenes-2026-07-28`, spec `specs/2026-07-28-atelier-scenes-design.md`, 8 commits). Bibliothèque de scènes Rive (`/api/scenes` : upload organisateur ≤ 10 Mo, magic bytes `RIVE`, refus multer en 400 contractuels), 4e mode visuel `scene` (`sceneId`/`sceneUrl` réécrits serveur, 400 si référence morte, self-healing au DELETE), `SceneDisplay.vue` (runtime `@rive-app/canvas` lazy en chunk séparé ~55 ko gzip, input `progress` ← `percentComplete`, fallback `none` prouvé au navigateur), carte admin + sélecteur de bibliothèque (i18n fr/en/he), **premier harnais vitest frontend** (contrôleur `sceneRuntime`, 6 tests). Atelier hors app `D:\Menora\atelier-scenes\` (runbook 5 étapes, SVGO, 22 seuils du POC, dépôt git dédié). Exécution deux étages Fable orchestrateur / Opus 5 exécuteurs (3 vagues, 7 périmètres disjoints). 217 tests backend + 6 frontend. Reste : fabrication **humaine** de `building.riv` (Rive Editor, runbook). |
