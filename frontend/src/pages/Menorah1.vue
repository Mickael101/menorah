...
// Generate ember particles that rise from the base - MORE FREQUENT
function startEmberGeneration(): void {
  emberInterval = window.setInterval(() => {
    if (Math.random() < 0.6) { // 60% chance per interval (was 30%)
      const ember = {
        id: nextEmberId++,
  ...
  }, 300); // 300ms interval (was 500ms)
}

function generateEmberBurst(): void {
  // Generate MORE embers on donation
  for (let i = 0; i < 15; i++) { // 15 embers (was 8)
    setTimeout(() => {
      const ember = {
        id: nextEmberId++,
        x: 40 + Math.random() * 20,
...
/* Background with deep black and warm glow - MORE INTENSE */
.background {
  position: absolute;
  inset: 0;
  pointer-events: none;
...
/* Embers/sparks rising continuously from base - LARGER AND MORE RED */
.embers-container {
  position: absolute;
  inset: 0;
  pointer-events: none;
...
.ember {
  ...
  width: 5px;
  height: 5px;
  background: radial-gradient(circle, #FFD54F 0%, #FF5722 50%, #E64A19 80%, #BF360C 100%);
  border-radius: 50%;
  box-shadow:
    0 0 10px #FF5722,
    0 0 20px rgba(255, 87, 34, 0.6),
    0 0 30px rgba(230, 74, 25, 0.4);
  animation: riseEmber 4s ease-out forwards;
}
