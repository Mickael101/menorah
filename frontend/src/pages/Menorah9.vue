...
function animateEntrance(): void {
  ...
  // Majestic divine ascension
  gsap.fromTo(svg,
    { opacity: 0, y: 40 },
    {
      opacity: 1,
      y: 0,
      duration: 2, // Stately and smooth
      ease: 'power3.out'
    }
  );
}

// Permanent subtle pulse - MAJESTIC AND WIDE
...
// Generate ember particles that rise from the base - GOLD DUST
function startEmberGeneration(): void {
  emberInterval = window.setInterval(() => {
    if (Math.random() < 0.4) {
      const ember = {
        id: nextEmberId++,
        x: 35 + Math.random() * 30, // Very wide spread
        y: 5
      };
